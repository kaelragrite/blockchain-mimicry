...
def broadcast_transaction():
    ...
    if not all(key in values for key in required):
        response = {'message': 'Missing required data.', 'data': required}
        return jsonify(response), 400
...
def add_transaction():
    ...
    signature = wallet.sign_transaction(values['recipient'], values['amount'])
    if blockchain.add_transaction(wallet.public_key, values['recipient'], signature, values['amount']):
        response = {
            'message': 'Successfully added a transaction.',
            'transaction': {
...
def mine():
    ...
    if block is not None and block != False:
...
def resolve_conflicts():
    response = {'message': 'Chain was replaced!'}\
        ...
        else {'message': 'Local chain was kept!'}
